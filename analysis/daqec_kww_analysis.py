...
if all_results:
    ...
    # Save to CSV
    all_df.to_csv(Path(REPORTS_DIR) / 'daqec_kww_results.csv', index=False)
    print(f"\nResults saved to {Path(REPORTS_DIR) / 'daqec_kww_results.csv'}")
else:
...
summary_text = "\n".join(summary_lines)
with open(Path(REPORTS_DIR) / 'daqec_summary.txt', 'w') as f:
    f.write(summary_text)
print(f"\nSaved: {Path(REPORTS_DIR) / 'daqec_summary.txt'}")
print("\n" + summary_text)
