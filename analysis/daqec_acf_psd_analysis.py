...
from config import DAQEC_DATA, FIGURES_DIR, REPORTS_DIR

DATA_DIR = Path(DAQEC_DATA)
...
if all_kww:
    ...
    kww_df.to_csv(Path(REPORTS_DIR) / 'daqec_refined_kww_results.csv', index=False)
    print(f"\nSaved: {Path(REPORTS_DIR) / 'daqec_refined_kww_results.csv'}")
...
summary_final = "\n".join(lines)
with open(Path(REPORTS_DIR) / 'daqec_refined_summary.txt', 'w') as f:
    f.write(summary_final)
print(f"\nSaved: {Path(REPORTS_DIR) / 'daqec_refined_summary.txt'}")
print("\n" + summary_final)
